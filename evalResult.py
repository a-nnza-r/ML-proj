...
def get_predicted(predicted, separator, outputColumnIndex, answers=defaultdict(lambda: defaultdict(defaultdict))):
...
def get_observed(observed, separator, outputColumnIndex):
...
def compare_observed_to_predicted(observed, predicted):
    ...
    entity_prec = correct_entity/total_predicted
    entity_rec = correct_entity/total_observed
    
    if abs(entity_prec + entity_rec ) < 1e-6:
        entity_f = 0
    else:
        entity_f = 2 * entity_prec * entity_rec / (entity_prec + entity_rec)
    # printResult('Entity', correct_entity, prec, rec)
    # print()

    sentiment_prec = correct_sentiment/total_predicted
    sentiment_rec = correct_sentiment/total_observed
    # printResult('Sentiment',correct_sentiment, prec, rec)
    
    if abs(sentiment_prec + sentiment_rec ) < 1e-6:
        sentiment_f = 0
    else:
        sentiment_f = 2 * sentiment_prec * sentiment_rec / (sentiment_prec + sentiment_rec)
